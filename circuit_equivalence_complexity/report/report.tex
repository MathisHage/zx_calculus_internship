...
When $\alpha$ is arbitrary, we prove \textsc{NP}-hardness by a reduction from \textsc{SUBSET-SUM}. We start by reminding its definition.
...
Now, we claim that $\textsc{SUBSET-SUM} \leq_p \textsc{ZX-REWRITE}$.

\begin{proof}
	Let the function $f: \textsc{SUBSET-SUM} \rightarrow \textsc{ZX-REWRITE}$ be defined as follows :

	\begin{algorithm}[!ht]
	    \caption{Reduction from $\textsc{SUBSET-SUM}$ to $\textsc{ZX-REWRITE}$}
	    \begin{algorithmic}[1]
		\Function{$f$}{$\left< S, t \right>$}
		    ...
		    \Else
		        ...
		        \State Let $D_1$ consist of $n$ green spiders disposed vertically, with the $i^{\text{th}}$ spider having phase $\frac{s_i}{\mathcal{N}}$ for $s_i \in S$.
		        \State Connect the left side of each spider in $D_1$ to a new $0$-phase green spider (one wire each), and the right side to another $0$-phase green spider (one wire each).
		        \State Let $D_2$ consist of two green spiders linked with $n$ wires, where the left spider has phase $\frac{\mathcal{N}-t}{\mathcal{N}}$ and the right spider has phase $\frac{t}{\mathcal{N}}$.
...
$$
\left< S, t \right> \in \textsc{SUBSET-SUM} \Leftrightarrow f(\left< S, t \right> ) = \left< D_1, D_2, k \right> \in \textsc{ZX-REWRITE}.
$$

Clearly if $t > \sum_{s \in S} s$, the reduction outputs a string that is not in \textsc{ZX-REWRITE}, as it is impossible to go from $D_1$ to $D_2$ in $k=0$ steps (they are not equal).
...
\begin{itemize}
	...
	\item[($\Leftarrow$)] It is possible to go from $D_1$ to $D_2$ in $n$ steps. Consider such a sequence of $n$ steps. As we go down from $n+2$ spiders to only $2$, and that we only have green spiders, we can only use spider fusion : using another rule would cause us to "loose time" while we only have $n$ moves allowed. Therefore, by creating a set $U$ containing all  $s \in S$ such that node $\frac{s_i}{\mathcal{N}}$ was slid to the right, we have that $\left< S, t \right> \in \textsc{SUBSET-SUM}$.
\end{itemize}


Therefore, $f$ is a valid reduction, which proves $\textsc{SUBSET-SUM} \leq_p \textsc{ZX-REWRITE}$.
