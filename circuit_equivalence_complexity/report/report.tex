...
\begin{IEEEkeywords}
Quantum Computing, ZX-Calculus, Proof Complexity, \textsc{NP}-hardness, \textsc{NP}, Reduction, Circuit Obfuscation, Blum-Shub-Smale Model.
\end{IEEEkeywords}
...
\begin{proof}
	...
	\begin{itemize}
		\item Phases are rational : they can perfectly be passed as inputs.
...
\begin{corollary}
	$\textsc{ZX-REWRITE}_F$ is \textsc {NP}-hard.
...
We now limit ourselves to the \textit{Clifford circuits}, i.e. the diagram will contain only phases that are multiples of $\pi / 2$, and will have as many outputs as they have inputs. Intuitively, the problem seems at most as hard as the case with only rational phases : not only is the number of possible phases now discrete ($\alpha \in \{0, \pi / 2, \pi, 3 \pi / 2\}$), but the set of rules is also reduced (the rules described in fig. (\ref{fig:zx_rules}) now suffice). Let $\textsc{ZX-REWRITE}_{CC}$ be the problem constrained to ZX-diagrams originating from Clifford circuits.

It is not too hard to see that the problem is in \textsc{NP} : with a proper labelling of the possible phases, alg. (\ref{alg:zxr_verifier}) still works as a polytime verifier for $\textsc{ZX-REWRITE}_{CC}$. The more interesting question would be to know if this problem could be in \textsc{P}. It turns out this question is still very complicated : after careful thinking, it seems unlikely that a polytime algorithm could be found easily to decide this problem (unless $\textsc{P} = \textsc{NP}$): how could we know that a valid sequence of steps is minimal without enumerating them all ? In the meantime, one still needs to find a different reduction than the one used previously to prove \textsc{NP}-hardness. Unfortunately due to a lack of time, this has not been achieved, and no polytime verifier has been identified. We nevertheless state our hypothesis :
