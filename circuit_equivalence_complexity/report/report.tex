...
$$
\textsc{ZX-REWRITE}_F := \{ \left< D_1, D_2, k \right> \in \textsc{ZX-REWRITE}\; | \; \text{we can go from } D_1 \text{ to } D_2 \text{ using the fusion rule only}\}.
$$ 
...
Otherwise, $k = n$ and :
\begin{itemize}
	\item[($\Rightarrow$)] $\left< S, t \right> \in \textsc{SUBSET-SUM} \implies \; \exists \; U \subseteq S$ s.t. $ \sum_{s \in U} s = t$. Start with $D_2$. By fusing every spider of the form $\frac{s}{\mathcal{N}}, s \in U$ with the right 0-phase spider, its phase becomes $\sum_{s \in U} \frac{s}{\mathcal{N}} = \frac{1}{\mathcal{N}} \sum_{s \in U} s = \frac{t}{\mathcal{N}}$. By fusing the remaining ones with the left 0-phase spider, its phase becomes $\sum_{s \notin U} s = \frac{\mathcal{N} - t}{\mathcal{N}}$. Hence we have recovered $D_2$, in $n$ steps exactly.
